...
# https://github.com/raess1/mini-cheetah-pybullet/blob/main/quadruped_ctrl/urdf/k3lso.urdf
...
def main():
    ...
    setJSMsg = JointState()
    pubPower = rospy.Publisher('/power', Float64, queue_size=1)
    msgPower = Float64()
    msgPower.data = 0
    powerWatt = 0
    powerWattHours = 0
    ...
    while not rospy.is_shutdown():
        ...
        setJSMsg.effort = [tau.contents.eff[0], tau.contents.eff[1], tau.contents.eff[2],
                             tau.contents.eff[3], tau.contents.eff[4], tau.contents.eff[5],
                             tau.contents.eff[6], tau.contents.eff[7], tau.contents.eff[8],
                             tau.contents.eff[9], tau.contents.eff[10], tau.contents.eff[11]]
        ...
        # simulation time
        now = rospy.Time.now()
        deltaSecs = (now - lastTime).to_sec() 
        durationSecs += deltaSecs  
        lastTime = now
        
        # watt-hours
        powerWatt = 0
        for i in range (0, 12):
            powerWatt += abs(tau.contents.eff[i]) * abs(joint_state[i][1]) 
        powerWattHours += powerWatt * deltaSecs / 3600 
        predictWattHours = powerWattHours * (3600 / durationSecs) 
        msgPower.data = powerWattHours

        if cnt % 5000 == 0:    
            print('seconds:', durationSecs, ' current power:', powerWatt, ' Watthours:', predictWattHours)
        
        pubPower.publish(msgPower)
        # ----------------------------------------------------------
